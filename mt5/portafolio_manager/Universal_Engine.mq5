//+------------------------------------------------------------------+
//|                                          Universal_Engine.mq5    |
//|                      Universal Engine v3.0                       |
//|         Auto-Detects Symbol Type & Applies Optimal Presets       |
//|             Confluence Ladder + Portfolio Integration            |
//+------------------------------------------------------------------+
#property copyright "Advanced Fibonacci Pro v7 - Universal Engine"
#property link      "https://github.com/GuidoAmbiorix"
#property version   "3.00"
#property description "Universal Engine: Auto-detection for Forex, Metals, Indices, Crypto"
#property strict
...
// UNIVERSAL ENGINE MODULES (v3.0)
#include "Include\Core\SymbolTypeDetector.mqh"
#include "Include\Config\UniversalConfig.mqh"
#include "Include\Core\PositionStateManager.mqh"
#include "Include\SessionOptimizer.mqh"  // For metals session filtering

//+------------------------------------------------------------------+
//| INPUT PARAMETERS                                                  |
//+------------------------------------------------------------------+
input group "======= UNIVERSAL ENGINE v3.0 ======="
input bool              InpUseAutoDetect = true;           // Auto-Detect Symbol Type
input int               InpManualSymbolType = 0;           // Manual Type (0=Auto, 1=Forex, 2=Metals, 3=Indices, 4=Crypto)

input group "======= PRESET OVERRIDES ======="
input bool              InpOverrideRiskBase = false;       // Override Preset Risk
input double            InpCustomRiskBase = 0.25;          // Custom Risk Base %
input bool              InpOverrideConfluence = false;     // Override Min Confluence
input int               InpCustomMinConfluence = 4;        // Custom Min Confluence
...
// UNIVERSAL ENGINE v3.0 OBJECTS
CSymbolTypeDetector    g_symbolDetector;
CUniversalConfig       g_universalConfig;
CPositionStateManager  g_positionManager;
CSessionOptimizer      g_sessionOptimizer;

// UNIVERSAL ENGINE GLOBALS
ENUM_SYMBOL_TYPE g_detectedSymbolType = SYMBOL_TYPE_UNKNOWN;
SymbolPreset g_activePreset;
bool g_useSessionOptimizer = false;
...
// DEPRECATED: Position tracking now handled by g_positionManager (CPositionStateManager)
// struct PositionState is defined in Include\Core\PositionStateManager.mqh
// No longer using g_states[] array - replaced with g_positionManager object
...
{
   Print("===================================================");
   Print("      UNIVERSAL ENGINE v3.0 - INITIALIZING");
   Print("===================================================");

   // ===== PHASE 1: SYMBOL TYPE AUTO-DETECTION =====
   if(!g_symbolDetector.Init(_Symbol)) {
      Print("ERROR: Failed to initialize Symbol Type Detector");
      return INIT_FAILED;
   }

   g_detectedSymbolType = g_symbolDetector.GetType();

   // Manual override if enabled
   if(!InpUseAutoDetect && InpManualSymbolType > 0 && InpManualSymbolType <= 4) {
      g_detectedSymbolType = (ENUM_SYMBOL_TYPE)(InpManualSymbolType - 1);
      Print("Manual Symbol Type Override: ", g_symbolDetector.GetTypeString());
   } else {
      Print("Auto-Detected Symbol Type: ", g_symbolDetector.GetTypeString());
   }

   // ===== PHASE 2: LOAD CONFIGURATION PRESET =====
   if(!g_universalConfig.Init()) {
      Print("ERROR: Failed to initialize Universal Config");
      return INIT_FAILED;
   }

   g_activePreset = g_universalConfig.GetPreset(g_detectedSymbolType);

   // Apply manual overrides
   if(InpOverrideRiskBase) {
      Print("Override: Risk Base ", g_activePreset.riskBase, "% -> ", InpCustomRiskBase, "%");
      g_activePreset.riskBase = InpCustomRiskBase;
   }

   if(InpOverrideConfluence) {
      Print("Override: Min Confluence ", g_activePreset.minConfluenceEntry, " -> ", InpCustomMinConfluence);
      g_activePreset.minConfluenceEntry = InpCustomMinConfluence;
   }

   Print("Active Preset Configuration:");
   Print("  Min Confluence: ", g_activePreset.minConfluenceEntry);
   Print("  Risk Base: ", g_activePreset.riskBase, "%");
   Print("  Max Risk: ", g_activePreset.maxRisk, "%");
   Print("  BE Threshold: ", g_activePreset.beThreshold_R, "R");
   Print("  Trail Start: ", g_activePreset.trailStart_R, "R");
   Print("  Session Optimizer: ", g_activePreset.useSessionOptimizer ? "ENABLED" : "DISABLED");

   // ===== PHASE 3: INITIALIZE POSITION STATE MANAGER =====
   if(!g_positionManager.Init(InpMagicNumber)) {
      Print("ERROR: Failed to initialize Position State Manager");
      return INIT_FAILED;
   }
   Print("Position State Manager: Initialized");

   // ===== PHASE 4: CONDITIONAL SESSION OPTIMIZER (METALS) =====
   g_useSessionOptimizer = g_activePreset.useSessionOptimizer;

   if(g_useSessionOptimizer) {
      if(!g_sessionOptimizer.Init(_Symbol, InpBrokerUTCOffset, g_activePreset.skipAsianSession, false)) {
         Print("WARNING: SessionOptimizer initialization failed");
         g_useSessionOptimizer = false;
      } else {
         Print("SessionOptimizer: ENABLED for ", g_symbolDetector.GetTypeString());
      }
   } else {
      Print("SessionOptimizer: DISABLED (not needed for ", g_symbolDetector.GetTypeString(), ")");
   }
   ...
   // Position state now handled by g_positionManager (initialized above)
...
void ResetTradeState()
{
   ...
   // Clear position state manager
   if(g_positionManager.GetPositionCount() > 0)
   {
      g_positionManager.Clear();
   }
}
...
void OnTick()
{
   ...
   // ===== UNIVERSAL ENGINE v3.0: METALS-SPECIFIC SESSION FILTERING =====
   if(g_useSessionOptimizer) {
      g_sessionOptimizer.Update();

      // METALS: Dynamic spread filter by session
      if(g_symbolDetector.IsMetals()) {
         double spreadUSD = GetSpreadInUSD();
         ENUM_TRADING_SESSION currentSession = g_sessionOptimizer.GetCurrentSession();

         double maxAllowedSpread = g_activePreset.maxSpreadUSD;

         // Adjust max spread by session quality
         if(currentSession == SESSION_LONDON) {
            maxAllowedSpread = 0.5;  // London: tight spread requirement
         } else if(currentSession == SESSION_NEW_YORK) {
            maxAllowedSpread = 0.8;  // NY: moderate
         } else if(currentSession == SESSION_LONDON_NY_OVERLAP) {
            maxAllowedSpread = 0.6;  // Overlap: prime time, moderate
         } else {
            maxAllowedSpread = 0.3;  // Asian/Other: very tight
         }

         if(spreadUSD > maxAllowedSpread) {
            static datetime lastSpreadLog = 0;
            if(TimeCurrent() - lastSpreadLog > 60) {
               Print("🪙 METALS SPREAD FILTER: ", DoubleToString(spreadUSD, 2), " USD > ",
                     DoubleToString(maxAllowedSpread, 2), " USD [", EnumToString(currentSession), "]");
      ...
      // METALS: Session score filter
      double sessionScore = g_sessionOptimizer.GetMetalsSessionScore();
      if(sessionScore < 3.0) {
         static datetime lastSessionLog = 0;
         if(TimeCurrent() - lastSessionLog > 300) {
            Print("🪙 METALS SESSION SCORE TOO LOW: ", DoubleToString(sessionScore, 1), " < 3.0");
            lastSessionLog = TimeCurrent();
         }
         return;
      }
   }
   // ===== END METALS-SPECIFIC FILTERING =====
...
bool ExecuteTrade(ENUM_ORDER_TYPE type, double riskPct, string label, ENTRY_QUALITY quality)
{
   ...
   if(trade.PositionOpen(_Symbol, type, lots, price, sl, tp, comment))
   {
      ...
      // Store in Position State Manager
      double dollarRisk = account.Equity() * (riskPct / 100.0);
      ConfluenceFactors entryFactors = (type == ORDER_TYPE_BUY) ? g_lastBuyFactors : g_lastSellFactors;

      g_positionManager.AddPosition(ticket, riskPct, dollarRisk, slDist, quality, entryFactors);
...
void ManagePositions()
{
   ...
   for(int i=g_positionManager.GetPositionCount()-1; i>=0; i--)
   {
      ulong ticket = g_positionManager.GetTicket(i);
      ...
      if(!PositionSelectByTicket(ticket))
      {
         ...
         if(HistorySelectByPosition(ticket))
         {
             ...
             double dollarRisk = g_positionManager.GetDollarRisk(i);
             ...
             if(InpEnableLearning && InpLogTradesToFile)
             {
                ...
                if(g_positionManager.GetPartialClosed(i)) exitReason += "_PARTIAL";
                ...
                ConfluenceFactors factors;
                g_positionManager.GetEntryFactors(i, factors);
         ...
         // Remove from Position State Manager
         g_positionManager.RemovePosition(ticket);
      }
   }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      // Get/Create State using Position Manager
      int sIdx = g_positionManager.FindIndex(ticket);

      // Create new state if not found (fallback - shouldn't happen if ExecuteTrade worked correctly)
      if(sIdx < 0)
      {
         double dollarRisk = account.Equity() * (g_activePreset.riskBase / 100.0);
         ConfluenceFactors emptyFactors;
         g_positionManager.AddPosition(ticket, g_activePreset.riskBase, dollarRisk, MathAbs(open - sl), EQ_GOOD, emptyFactors);
         Print("Warning: Created fallback position state for ticket ", ticket, " - R-calculations may be approximate");
         sIdx = g_positionManager.FindIndex(ticket);
      }

      if(sIdx < 0) {
         Print("ERROR: Failed to get or create position state for ticket ", ticket);
         continue;
      }

      double risk = g_positionManager.GetInitialRisk(sIdx);  // kept for AdaptiveExitManager call below
      ...
      double initialSLDist = g_positionManager.GetInitialSLDist(sIdx);
      double slDist = (initialSLDist > _Point) ? initialSLDist
                    : (sl > 0)                  ? MathAbs(open - sl)
                                                : risk;
      double profitR = (slDist > _Point) ? rawProfit / slDist : 0;

      ENTRY_QUALITY quality = g_positionManager.GetQuality(sIdx);
      ...
      if(InpTrailingMode >= 1)
      {
         ...
         if(!g_positionManager.GetPartialClosed(sIdx) && profitR >= partTP)
         {
            ...
            if(closeVol >= minV && (vol - closeVol) >= minV)
            {
               ...
               {
                  g_positionManager.SetPartialClosed(ticket, true);
...
void OnTrade()
{
   ...
   for(int i = 0; i < dealCount; i++)
   {
       ...
       // FIX: Calculate accurate R-multiple using stored dollarRisk from Position Manager
       ENTRY_QUALITY quality = EQ_GOOD;
       double dollarRisk = 0;
       bool foundState = false;

       int stateIdx = g_positionManager.FindIndex(ticket);
       if(stateIdx >= 0)
       {
          dollarRisk = g_positionManager.GetDollarRisk(stateIdx);
          quality = g_positionManager.GetQuality(stateIdx);
          foundState = true;
       }
...
//+------------------------------------------------------------------+
//| Get Spread in USD (for metals filtering)                         |
//+------------------------------------------------------------------+
double GetSpreadInUSD()
{
   CSymbolInfo symInfo;
   ...
   // For metals (XAUUSD, XAGUSD), spread in points translates directly to USD
   // because point value represents price change in USD
   double spreadUSD = spread;
