...
//|    Generated at 04/09/2026 23:18
//| 
//|    Backtested on XAUUSD / H1, 2013.05.01 - 2020.07.01 
//+------------------------------------------------------------------+
...
input string CustomComment = "Xau3";
...
input int ExitAfterBars1 = 20;        //ExitAfterBars1 (reducido de 42 - menos exposicion)
input double MoveSL2BE1 = 25;        //MoveSL2BE1
input double SL2BEAddPipsCoef1 = 1;        //SL2BEAddPipsCoef1
input double ProfitTargetCoef1 = 2.9;        //ProfitTargetCoef1
input double StopLossCoef1 = 3.6;        //StopLossCoef1
input int ExitAfterBars2 = 10;        //ExitAfterBars2
input double MoveSL2BE2 = 230;        //MoveSL2BE2
input double SL2BEAddPipsCoef2 = 2.8;        //SL2BEAddPipsCoef2
input double ProfitTargetCoef2 = 3.1;        //ProfitTargetCoef2
input double StopLossCoef2 = 3.2;        //StopLossCoef2
...
input double mmMaxLots = 1.0;
input double mmMultiplier = 1.0;
input double mmStep = 0.01;
  
  
input double InitialCapital = 5000; //InitialCapital (0 means whole account balance). Used in some MM methods.
...
input int MaxTradesPerDay = 1;  // Maximo 1 trade por dia (calidad sobre cantidad)
...
input string sqfi          = "----------- Quality Signal Filters -----------";
input bool   UseTrendFilter   = true;    // Filtro de tendencia via EMA (solo operar a favor)
input int    MA_Fast_Period   = 50;      // Periodo EMA rapida
input int    MA_Slow_Period   = 200;     // Periodo EMA lenta
input bool   UseATRVolFilter  = true;    // Filtro volatilidad ATR (evitar mercado muerto o noticias)
input double ATR_Vol_Min      = 0.5;     // ATR minimo como factor del ATR(159) promedio (0=desactivado)
input double ATR_Vol_Max      = 3.0;     // ATR maximo como factor del ATR(159) promedio (0=desactivado)
input bool   UseSessionFilter = true;    // Solo operar en sesiones activas London/NY
input string LondonOpen       = "07:00"; // Apertura sesion London (hora servidor broker)
input string LondonClose      = "12:00"; // Cierre sesion London
input string NewYorkOpen      = "13:00"; // Apertura sesion Nueva York
input string NewYorkClose     = "17:00"; // Cierre sesion Nueva York
input bool   AllowShortTrades = true;    // Permitir trades SHORT (estaban rotos, ahora corregidos)
...
#define ATR_1 0     //iCustom(NULL, 0, "SqATR", 24)
#define ATR_2 1     //iCustom(NULL, 0, "SqATR", 30)
#define ATR_3 2     //iCustom(NULL, 0, "SqATR", 40)
#define ATR_4 3     //iCustom(NULL, 0, "SqATR", 159)
#define ATR_5 4     //iCustom(NULL, 0, "SqATR", 25)
#define HEIKENASHI_1 5     //255
#define MA_FAST 6          //iMA(NULL, 0, MA_Fast_Period, 0, MODE_EMA, PRICE_CLOSE)  - EMA rapida filtro tendencia
#define MA_SLOW 7          //iMA(NULL, 0, MA_Slow_Period, 0, MODE_EMA, PRICE_CLOSE)  - EMA lenta filtro tendencia
...
//+------------------------------------------------------------------+
// Devuelve true si la hora actual del servidor esta dentro de la
// sesion London (LondonOpen-LondonClose) o NY (NewYorkOpen-NewYorkClose)
//+------------------------------------------------------------------+
bool IsActiveTradingSession() {
   MqlDateTime now;
   TimeToStruct(TimeCurrent(), now);
   int curMin = now.hour * 60 + now.min;

   // Parsear strings HH:MM
   int loH = (int)StringToInteger(StringSubstr(LondonOpen,  0, 2));
   int loM = (int)StringToInteger(StringSubstr(LondonOpen,  3, 2));
   int lcH = (int)StringToInteger(StringSubstr(LondonClose, 0, 2));
   int lcM = (int)StringToInteger(StringSubstr(LondonClose, 3, 2));
   int nyoH = (int)StringToInteger(StringSubstr(NewYorkOpen,  0, 2));
   int nyoM = (int)StringToInteger(StringSubstr(NewYorkOpen,  3, 2));
   int nycH = (int)StringToInteger(StringSubstr(NewYorkClose, 0, 2));
   int nycM = (int)StringToInteger(StringSubstr(NewYorkClose, 3, 2));

   bool inLondon  = (curMin >= loH  * 60 + loM  && curMin < lcH  * 60 + lcM);
   bool inNewYork = (curMin >= nyoH * 60 + nyoM && curMin < nycH * 60 + nycM);

   return inLondon || inNewYork;
}

//+------------------------------------------------------------------+
// Devuelve true si la volatilidad ATR(24) esta dentro del rango
// aceptable respecto al ATR(159) de largo plazo
//+------------------------------------------------------------------+
bool IsVolatilityInRange() {
   double atrShort = sqGetIndicatorValue(ATR_1, 1);  // ATR(24)  - volatilidad reciente
   double atrLong  = sqGetIndicatorValue(ATR_4, 1);  // ATR(159) - promedio historico
   if(atrLong <= 0 || atrShort <= 0) return true;    // sin datos: no filtrar
   double ratio = atrShort / atrLong;
   if(ATR_Vol_Min > 0 && ratio < ATR_Vol_Min) return false; // mercado dormido
   if(ATR_Vol_Max > 0 && ratio > ATR_Vol_Max) return false; // noticias / spike
   return true;
}
...
if (_sqIsBarOpen == true) {
     ...
     // --- Filtro de tendencia: EMA rapida vs EMA lenta (barra 1 = ultima cerrada)
     double _emaFast = sqGetIndicatorValue(MA_FAST, 1);
     double _emaSlow = sqGetIndicatorValue(MA_SLOW, 1);
     bool _trendBull = !UseTrendFilter || (_emaFast > _emaSlow);
     bool _trendBear = !UseTrendFilter || (_emaFast < _emaSlow);

     // --- Filtro de volatilidad ATR
     bool _volOK = !UseATRVolFilter || IsVolatilityInRange();

     // --- Filtro de sesion (London / New York)
     bool _sessOK = !UseSessionFilter || IsActiveTradingSession();

     // --- No abrir si ya hay posicion abierta en este magic
     bool _noPos = !sqMarketPositionIsLong(MagicNumber, "Current", "")
                && !sqMarketPositionIsShort(MagicNumber, "Current", "");

     // --- Señales finales (calidad sobre cantidad)
     LongEntrySignal  = _trendBull && _volOK && _sessOK && _noPos;
     ShortEntrySignal = AllowShortTrades && _trendBear && _volOK && _sessOK && _noPos;
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Market
      openPrice = 0;  //Open at current market price
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL, openPrice, 2, StopLossCoef2 * sqGetIndicatorValue(ATR_5, 1)), "Current");
      ...
      if(_ticket > 0) {
         ...
         //sqCheckSLPT(_ticket, sl, pt);
         // MoveSL2BE initialization
         sqSetMoveSL2BE(_ticket, "MoveSL2BE2", SLPTTYPE_RANGE);
         sqSetSL2BEAddPips(_ticket, "SL2BEAddPipsCoef2 * sqGetIndicatorValue(ATR_4, 1)");
...
//------------------------
...
)
   {
      // Action #1
        // Close all positions
      sqCloseAllPositions("Current", MagicNumber, 1, "");
...
bool initIndicators(){
   ...
   indicatorHandles[ATR_1] = iCustom(NULL, 0, "SqATR", 24);
   ...
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_4] = iCustom(NULL, 0, "SqATR", 159);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_5] = iCustom(NULL, 0, "SqATR", 25);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HEIKENASHI_1] = 255;

   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MA_FAST] = iMA(NULL, 0, MA_Fast_Period, 0, MODE_EMA, PRICE_CLOSE);

   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MA_SLOW] = iMA(NULL, 0, MA_Slow_Period, 0, MODE_EMA, PRICE_CLOSE);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("MoveSL2BE2")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), MoveSL2BE2));
   }
   if(idHash == sqStringHash("SL2BEAddPipsCoef2 * sqGetIndicatorValue(ATR_4, 1)")) {
      return (SL2BEAddPipsCoef2 * sqGetIndicatorValue(ATR_4, 1));
   }
   if(idHash == sqStringHash("ProfitTargetCoef2 * sqGetIndicatorValue(ATR_2, 1)")) {
      return (ProfitTargetCoef2 * sqGetIndicatorValue(ATR_2, 1));
   }
   if(idHash == sqStringHash("StopLossCoef2 * sqGetIndicatorValue(ATR_5, 1)")) {
      return (StopLossCoef2 * sqGetIndicatorValue(ATR_5, 1));
   }
