...
// PHASE 2: MTF moved to custom indicator
...
int OnInit()
{
   ...
   if(InpUseMTF)
   {
      ...
      if(hMTF_Confluence == INVALID_HANDLE)
      {
         Print("ERROR: Failed to create MTF_Confluence indicator");
         return INIT_FAILED;
      }

      if(!WaitForIndicator(hMTF_Confluence, 3))
      {
         Print("ERROR: MTF_Confluence indicator failed to initialize");
         return INIT_FAILED;
      }
   ...
   if(InpUseSMC)
   {
      hICT_Advanced = iCustom(_Symbol, PERIOD_CURRENT,
         "Indicators\\ICT_Advanced");

      if(hICT_Advanced == INVALID_HANDLE)
      {
         Print("ERROR: Failed to create ICT_Advanced indicator");
         return INIT_FAILED;
      }

      if(!WaitForIndicator(hICT_Advanced, 3))
      {
         Print("ERROR: ICT_Advanced indicator failed to initialize");
         return INIT_FAILED;
      }
   ...
   hDivergence = iCustom(_Symbol, PERIOD_CURRENT,
      "Indicators\\Divergence",
      InpRSI_Period, 10);

   if(hDivergence == INVALID_HANDLE)
   {
      Print("ERROR: Failed to create Divergence indicator");
      return INIT_FAILED;
   }

   if(!WaitForIndicator(hDivergence, 3))
   {
      Print("ERROR: Divergence indicator failed to initialize");
      return INIT_FAILED;
   }
...
void OnTick()
{
   ...
   {
      double tradingAllowedBuf[1];
      if(CopyBuffer(hNews_Filter, 4, 0, 1, tradingAllowedBuf) > 0)
      {
         if(tradingAllowedBuf[0] == 0.0) return;  // Trading not allowed
      }
   }
   ...
   if(g_positionCount == 0)
   {
      ...
      if(InpUseKelly)
      {
         ...
         // PHASE 4: Get news risk multiplier from News_Filter indicator
         ...
         if(InpUseNewsFilter && hNews_Filter != INVALID_HANDLE)
         {
               ...
               CopyBuffer(hNews_Filter, 1, 0, 1, minToNewsBuf) > 0)
            {
               ...
               else if(minToNewsBuf[0] <= InpNewsMinutesBefore * 3) newsMultiplier = 0.75;
            }
         }
...
void UpdateModules()
{
   // PHASE 1: SMC modules now handled by custom indicators
   // No manual updates needed - indicators update automatically

   // PHASE 2: MTF now handled by custom indicator
   // No manual updates needed - indicators update automatically

   // OPTIMIZATION: Update filters only if enabled
   // PHASE 4: News filter update is handled automatically by indicator
...
void UpdateDashboard()
{
   ...
   // Check for blocks
   // PHASE 4: Check trading allowed from News_Filter indicator
   if(InpUseNewsFilter && hNews_Filter != INVALID_HANDLE)
   {
         ...
         CopyBuffer(hNews_Filter, 3, 0, 1, volSpikeBuf) > 0)
      {
         ...
         else if(tradingAllowedBuf[0] == 0.0) tradingStatus = "NEWS BLOCKED";
      }
   }
   ...
   // News info
   if(InpUseNewsFilter)
      // PHASE 4: Get news status from News_Filter indicator
      if(hNews_Filter != INVALID_HANDLE)
      {
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| PHASE 4: Get Active Killzone from Killzone_Detector indicator    |
//+------------------------------------------------------------------+
ENUM_KILLZONE GetActiveKillzone()
{
   if(!InpUseKillzoneFilter) return KILLZONE_NONE;

   // PHASE 4: Read from Killzone_Detector indicator (Buffer 0)
   if(hKillzone_Detector != INVALID_HANDLE)
   {
      double kzBuf[1];
...
bool CheckKillzone()
{
   ...
   // PHASE 4: Read from Killzone_Detector indicator (Buffer 1: IsActive)
   if(hKillzone_Detector != INVALID_HANDLE)
   {
      double activeBuf[1];
