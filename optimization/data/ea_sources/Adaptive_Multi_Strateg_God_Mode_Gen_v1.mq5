...
void OnTick()
{
   ...
   // Features prepared for Semantic compatibility with GPLearn
   double feat_RSI = rsi[0];
   double feat_ATR = atr[0];
   ...
   double feat_Mom = (iClose(_Symbol, 0, 0) - iClose(_Symbol, 0, 10)); // Simple Momentum
    ...
    double geneticSignal = 0.0; // Placeholder for X0
    // ----------------------
    // [[GENETIC_LOGIC_END]]
    
   
   // Genetic Execution Logic (Priority Alpha)
   if(geneticSignal > 0.001 || geneticSignal < -0.001)
   {
       if(!HasOpenTrade("Genetic"))
       {
           double slDist = 2.0 * atr[0];
           double tpDist = 4.0 * atr[0];
           double price = (geneticSignal > 0) ? SymbolInfoDouble(_Symbol, SYMBOL_ASK) : SymbolInfoDouble(_Symbol, SYMBOL_BID);
           
           ENUM_ORDER_TYPE orderType = (geneticSignal > 0) ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
           double sl = (geneticSignal > 0) ? price - slDist : price + slDist;
           double tp = (geneticSignal > 0) ? price + tpDist : price - tpDist;
           
           ExecuteTrade(orderType, sl, tp, "Genetic_Alpha", InpRisk_Per_Trade);
           return; // Skip standard strategies if Alpha triggers
       }
   }
   // -------------------------------------
...
void ExecuteTrade(ENUM_ORDER_TYPE type, double sl, double tp, string comment, double riskPct = 0.0)
{
   ...
   // [REMOVED] Hardcoded XAU cap. Now using User Input ONLY.
   
   double volume = CalculateLotSizeWithRisk(slDist, effectiveRisk);

   // Apply Max Lot Cap (User Input)
   if(volume > InpMaxLot_Per_Trade) volume = InpMaxLot_Per_Trade;
   ...
   {
       // [REMOVED] Auto-Reduce Logic. 
       // If margin is insufficient, we just FAIL/WARN, but we don't skew the test results silently.
       if(InpDebugMode) Print("⛔ MARGIN FAIL: Need ", DoubleToString(marginRequired, 2), " | Free ", DoubleToString(freeMargin, 2));
       return;
   }
...
bool IsContextFavorable(bool isBuy)
{
   ...
   double pricePosition = (current - low) / range; // 0.0 = Low, 1.0 = High
   ...
   if(isRangeMode)
   {
       ...
       if(isBuy && pricePosition > 0.75)  { if(InpDebugMode) Print("⛔ Context Veto: Buying at Range High"); return false; }
       if(!isBuy && pricePosition < 0.25) { if(InpDebugMode) Print("⛔ Context Veto: Selling at Range Low"); return false; }
   }
   else if(isTrendMode)
   {
   ...
   else
   {
       ...
       if(isBuy && pricePosition > 0.90) { if(InpDebugMode) Print("⛔ Context Veto: Buying Extreme High"); return false; }
       if(!isBuy && pricePosition < 0.10) { if(InpDebugMode) Print("⛔ Context Veto: Selling Extreme Low"); return false; }
   }
