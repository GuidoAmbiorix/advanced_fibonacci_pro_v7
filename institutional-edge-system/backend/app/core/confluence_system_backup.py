...
class EnhancedConfluenceScorer:
    ...
    def score_continuation(
        ...
        """
        Score a CONTINUATION trade setup

        Continuation = BOS + Price retracement to support (OB/FVG/Fib)
        """
        breakdown = ConfluenceBreakdown(trade_type="CONTINUATION")

        # Structure (MANDATORY for continuation)
        if has_bos:
    ...
    def score_reversal(
        ...
        """
        Score a REVERSAL trade setup

        Reversal = CHoCH + Liquidity grab + Fibonacci level
        Higher requirements than continuation
        """
        ...
        # Fibonacci (REQUIRED for reversals)
        breakdown.fibonacci_score = fib_score
        if fib_score > 0:
            breakdown.factors['Fibonacci'] = fib_score

        # Liquidity (REQUIRED for reversals)
        if has_liquidity_sweep:
            breakdown.liquidity_score += self.WEIGHTS['LIQUIDITY_SWEEP']
            breakdown.factors['Liquidity Sweep'] = self.WEIGHTS['LIQUIDITY_SWEEP']
