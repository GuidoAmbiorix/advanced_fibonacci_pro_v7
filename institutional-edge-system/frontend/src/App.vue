  ...
  <div class="min-h-screen bg-background font-sans text-text-primary flex">
    ...
    <aside v-if="!isStreamMode" class="w-64 bg-surface border-r border-slate-200 flex flex-col fixed h-full z-40 transition-all duration-300" :class="isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'">
      ...
      <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
        <!-- Trading (main view) -->
        <router-link to="/backtest" class="flex items-center space-x-3 px-4 py-3 rounded-lg font-bold text-sm transition-colors" :class="$route.path === '/backtest' || $route.path === '/' ? 'bg-accent/10 text-accent' : 'text-slate-500 hover:bg-slate-50 hover:text-primary'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
          <span>Trading</span>
        </router-link>

        <!-- Accounts -->
        <router-link to="/accounts" class="flex items-center space-x-3 px-4 py-3 rounded-lg font-bold text-sm transition-colors" :class="$route.path === '/accounts' ? 'bg-accent/10 text-accent' : 'text-slate-500 hover:bg-slate-50 hover:text-primary'">
