...
async function loadHeatmap() {
  loading.value = true
  try {
    const response = await axios.get(`/api/backtest/${props.sessionId}/heatmap`, {
      params: { bin_size: binSize.value }
    })
...
function getBarStyle(bin: any) {
  const maxEntries = Math.max(...heatmapData.value.bins.map((b: any) => b.entry_count))
