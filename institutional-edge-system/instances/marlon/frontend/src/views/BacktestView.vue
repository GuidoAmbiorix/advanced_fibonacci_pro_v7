<template>
  <div class="p-6 space-y-6 min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
    ...
    <!-- TABS NAVIGATION                                                         -->
    <!-- ═══════════════════════════════════════════════════════════════════════ -->
    <div class="flex space-x-6 border-b border-gray-700/50 mb-6">
       <button 
         @click="currentTab = 'dashboard'"
         class="pb-3 text-sm font-bold transition-all border-b-2"
         :class="currentTab === 'dashboard' ? 'text-blue-400 border-blue-400' : 'text-gray-400 border-transparent hover:text-gray-300'"
       >
         📊 Dashboard
       </button>
       <button 
         ...
         class="pb-3 text-sm font-bold transition-all border-b-2"
         :class="currentTab === 'grid' ? 'text-cyan-400 border-cyan-400' : 'text-gray-400 border-transparent hover:text-gray-300'"
       >
         📈 Chart Grid (TradingView)
       </button>
       <button 
         @click="currentTab = 'analysis'"
         class="pb-3 text-sm font-bold transition-all border-b-2"
         :class="currentTab === 'analysis' ? 'text-purple-400 border-purple-400' : 'text-gray-400 border-transparent hover:text-gray-300'"
       >
         🔥 Heatmap Analysis
       </button>
    </div>
    ...
    <!-- VIEW: GRID -->
    <div v-if="currentTab === 'grid'" class="h-[800px] border border-gray-700/50 rounded-xl overflow-hidden shadow-2xl bg-[#0f111a] mt-6">
        <SlotGridView />
    </div>

    <!-- VIEW: ANALYSIS -->
    <div v-if="currentTab === 'analysis'" class="space-y-6 mt-6">
        <EntryZoneHeatmap :sessionId="1" />
    </div>
...
// import UnifiedTradingChart from '../components/tradingview/UnifiedTradingChart.vue' // Used inside SlotGridView
...
const deleteSlot = async (slotId) => {
  ...
  if (slot.dbId) {
    try {
      await api.deleteSlot(slot.dbId)
  ...
  saveTimeout = setTimeout(async () => {
    try {
      const payload = {
        ...
        // Institutional
        confirmation_timeframe: slot.confirmation_timeframe || null,  // Must use null for optional
        trading_session: slot.trading_session || 'ALL',
        session_end_action: slot.session_end_action || 'HOLD',
